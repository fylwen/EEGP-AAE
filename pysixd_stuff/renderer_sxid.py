...
void main() {
    ...
    v_light_pos = normalize(u_light_eye_pos - v_eye_pos); // Vector to the light
...
def draw_color(shape, vertex_buffer, index_buffer, texture, mat_model, mat_view,
               mat_proj, ambient_weight, bg_color, shading,light_pos,ambient_k,diffuse_k,specular_k):
    ...
    program['u_light_eye_pos'] = light_pos # Camera origin
...
def render(model, im_size, K, R, t, clip_near=100, clip_far=2000,
           ...
           shading='phong', mode='rgb+depth',random_light=True):
