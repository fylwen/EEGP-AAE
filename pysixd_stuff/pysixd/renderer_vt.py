...
void main() {
    ...
    v_light_pos = normalize(u_light_pos - v_eye_pos); // Vector to the light
    v_normal = normalize(u_nm * vec4(a_normal, 1.0)).xyz; // Normal in eye coords.
    v_reflect=normalize(reflect(-v_light_pos,v_normal));//Reflect vector to the light
    shiness=32;
...
void main() {
    ...
    float light_diffuse_w = max(dot(normalize(v_light_pos), normalize(face_normal)), 0.0);
...
uniform float u_light_ambient_w;
uniform float u_light_ambient_k;
uniform float u_light_diffuse_k;
uniform float u_light_specular_k;
uniform sampler2D u_texture;
uniform int u_use_texture;
...
varying float shiness;
void main() {
    float light_diffuse_w = max(dot(normalize(v_light_pos), normalize(v_normal)), 0.0);
    float light_specular_w=pow(max(dot(normalize(v_eye_pos),normalize(v_reflect)),0.0),shiness);
    if(light_diffuse_w<=0) light_specular_w=0.0;

    float light_w = u_light_ambient_k*u_light_ambient_w + 
                    u_light_diffuse_k*light_diffuse_w + 
                    u_light_specular_k*light_specular_w;
    if(light_w > 1.0) light_w = 1.0;
...
#Visible point vertex
_visibpoint_vertex_code = """
uniform mat4 u_mv;
uniform mat4 u_mvp;
attribute vec3 a_position;
attribute vec3 a_color;
varying float v_eye_depth;
varying vec3 vis_position;
...
void main() {
    ...
    // OpenGL Z axis goes out of the screen, so depths are negative
    v_eye_depth = -v_eye_pos.z;
    vis_position=a_position;
}
"""

# Visible point fragment shader
# -------------------------------------------------------------------------------
_visibpoint_fragment_code = """
varying float v_eye_depth;
varying vec3 vis_position;

void main() {
    gl_FragColor = vec4(vis_position, 1.0);
}
"""
...
def draw_color(shape, vertex_buffer, index_buffer, texture, mat_model, mat_view,
               mat_proj, bg_color, shading, light_pos, ambient_k, diffuse_k, specular_k):
    ...
    program['u_light_pos'] = light_pos  # Camera origin
    program['u_light_ambient_w'] = 1.0
    program['u_light_ambient_k'] = ambient_k
    program['u_light_diffuse_k'] = diffuse_k
    program['u_light_specular_k'] = specular_k
...
def draw_visib_point(shape, vertex_buffer, index_buffer, mat_model, mat_view, mat_proj):
    program = gloo.Program(_visibpoint_vertex_code, _visibpoint_fragment_code)
    program.bind(vertex_buffer)
    program['u_mv'] = _compute_model_view(mat_model, mat_view)
    program['u_mvp'] = _compute_model_view_proj(mat_model, mat_view, mat_proj)
    ...
    gl.glReadPixels(0, 0, shape[1], shape[0], gl.GL_RGBA, gl.GL_FLOAT, depth)
    depth.shape = shape[0], shape[1], 4
    visib_points = depth[::-1, :]
    visib_points=visib_points[:,:,:-1]
...
# -------------------------------------------------------------------------------
def render_phong(model, im_size, K, R, t, clip_near=100, clip_far=2000,
           texture=None, surf_color=None, bg_color=(0.0, 0.0, 0.0, 0.0),
           mode='rgb+depth', random_light=True,phong={'ambient': 0.4, 'diffuse': 0.8, 'specular': 0.3}):
    shading='phong'
    if random_light:
        light_pos = 1000. * np.random.random(3)
        ambient_k = phong['ambient']
        diffuse_k = phong['diffuse'] + 0.1 * (2 * np.random.rand() - 1)
        specular_k = phong['specular'] + 0.1 * (2 * np.random.rand() - 1)
    else:
        light_pos = np.array([400., 400., 400.])
        ambient_k = phong['ambient']
        diffuse_k = phong['diffuse']
        specular_k = phong['specular']
    ...
    # Rendering
    # ---------------------------------------------------------------------------
    render_rgb = mode in ['rgb', 'rgb+depth', 'rgb+depth+coord']
    render_depth = mode in ['depth', 'rgb+depth','rgb+depth+coord']
    render_coord=mode in ['coord','rgb+depth+coord']
    ...
    def on_draw(dt):
        ...
        if render_rgb:
            ...
            rgb = draw_color(shape, vertex_buffer, index_buffer, texture, mat_model,
                             mat_view, mat_proj, bg_color, shading,
                             light_pos=light_pos, ambient_k=ambient_k, diffuse_k=diffuse_k, specular_k=specular_k)
        if render_depth:
            # Render depth image
            global depth
            depth = draw_depth(shape, vertex_buffer, index_buffer, mat_model,
                               mat_view, mat_proj)
        if render_coord:
            #Render coordinate of visible coordinates
            global visib_pts
            visib_pts = draw_visib_point(shape, vertex_buffer, index_buffer, mat_model,
                               mat_view, mat_proj)
    ...
    if mode == 'rgb':
        ...
        return visib_pts
    elif mode == 'rgb+depth+coord':
        return rgb,depth,visib_pts
    else:
